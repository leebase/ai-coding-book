...
Open Antigravity and start a new agent session. Type:
...
> **Free Tier Note:** Each correction is another prompt. Each prompt may fix one test and break another. You are debugging a multi-change diff without a clear view of which change caused which failure.
...
The solution is to plan your increments before you open Antigravity, the same way you write a requirement before you build a feature.
...
Start a new agent session (new conversation icon at the top of the sidebar). Provide the full context of the current state, then the increment spec:
...
# Check tasks.json directly — open it in the file explorer
...
Start a new agent session (new conversation icon). Give the agent the current state and increment 2:
...
Start a new agent session (new conversation icon). Give the agent the full current state and increment 3:
