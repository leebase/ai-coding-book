...
In the Antigravity file explorer (left panel), right-click and select New File. Name it `context.md`. Copy and fill in this template:
...
Open Antigravity and start a new agent session (new conversation icon at the top of the agent sidebar). Paste only this:
...
Start a new agent session (new conversation icon at the top of the agent sidebar). Open `context.md` in the Antigravity file explorer, copy its full contents, and send this prompt — replacing the bracketed placeholder with the actual file contents:
...
> **Free Tier Note:** This exercise does not consume much of your rate limit — you are asking the agent to read and respond, not to write code. Run it deliberately; pay attention to which decisions the agent correctly recalls and which it misses.
