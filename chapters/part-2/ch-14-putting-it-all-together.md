...
**Multi-agent coordination** via Manager View: parallel agents when scopes are clean, serialization when they are not, human merge review before any session closes.
...
Before opening Antigravity, create or update `sprint-plan.md`. Answer the five questions:
...
Open a new agent session (new conversation icon at the top of the agent sidebar). Open `start-session.md` in the Antigravity file explorer, copy its contents, and paste at the top of your first message. Append the requirement below it. Specify the autonomy mode explicitly — add a line like "Mode: Implement and verify" or "Mode: Stage by stage" so the agent knows whether to pause.
