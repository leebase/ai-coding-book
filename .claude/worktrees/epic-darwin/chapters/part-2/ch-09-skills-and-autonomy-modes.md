...
Create the skill file in your task manager project. In the Antigravity file explorer (left panel), right-click and select New File. Name it `start-session.md`. Copy the template above, updating `[CURRENT_COUNT]` to `14` (your current test count after Chapter 8).

Save with Cmd+S or Ctrl+S.

> **Note:** The skill file lives in your project directory alongside `tasks.py` and `decisions.md`. It is not a special Antigravity file type — it is a plain markdown file you reference manually at the start of each session by copying its contents into your prompt.
...
Open Antigravity and start a new agent session (new conversation icon at the top of the agent sidebar). Type from memory:
...
> **Free Tier Note:** Run the tests anyway to see the count:
...
Start a new agent session (new conversation icon at the top of the agent sidebar). This time, open `start-session.md` in the Antigravity file explorer, copy its contents, and paste them at the top of your prompt. Then add the feature requirement below:
