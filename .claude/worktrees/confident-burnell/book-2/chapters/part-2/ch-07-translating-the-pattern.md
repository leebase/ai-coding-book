...
**Shared memory.** `requirement.md` is the equivalent of `brief.md` — the document every stage reads from. It does not change between stages. If the requirement changes mid-sprint, the sprint stops and restarts. That constraint matters more in code than in writing, because code has dependencies that prose does not.
